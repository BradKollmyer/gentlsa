...
- Fix `--timeout` not being a real deadline within a connection: socket timeouts are per-syscall and the negotiation readers consume a byte at a time, so a peer trickling bytes just under the limit could run far past the budget — measured at 58s against `--timeout 5`. Every read and write is now re-armed to the remaining budget, and an exhausted budget reports `timed out after Ns` instead of a bare `Resource temporarily unavailable`. SMTP continuation lines and IMAP untagged responses are also capped
- Fix XMPP sending the connect host as the stream's `to=` attribute and SNI instead of the XMPP domain (RFC 6120 §4.7.2, RFC 7673 §6). With the usual SRV indirection — `conversations.im` served by `xmpp.conversations.im` — the server answered `<host-unknown/>` and closed the connection, so no TLSA record could be generated. Both are now the zone, so `generate example.com 5222 --hostname xmpp` opens the stream to `example.com` while still publishing at `_5222._tcp.xmpp.example.com`
- Fix SMTP `EHLO` sending the bare label `gentlsa`, which RFC 5321 §4.1.1.1 does not permit: hardened servers (Postfix `reject_non_fqdn_helo_hostname`) answered `504 need fully-qualified hostname`, so no TLSA record could be generated for those hosts at all. It now sends the connection's own source address as an address literal (RFC 5321 §4.1.3)
- Fix `--azure` discarding `resource_group` (and reporting a confusing error for a bad `credentials` path) when credentials came from a service-principal JSON file: the group was dropped, so zone lookups fell back to a subscription-wide list that a resource-group-scoped service principal cannot read, failing every publish with `403 AuthorizationFailed`. A `credentials` path that does not exist is now an error instead of a silent fallback, and `azure --info` prints the resource group in effect
- Fix one unreachable host aborting a whole `generate` or `prune` run: with `--mx` (or a port list), a single dead exchange meant nothing was published at all, not even for the healthy hosts, and `--json` emitted no report. Failures are now recorded per host (`error`, with `certificate`/`live` omitted) and the remaining hosts are processed. Also fix the exit code being overwritten by the last host, which could report success after an earlier failure
- Fix `rollover --reload` always failing in the prune phase with "timed out after Ns" and leaving the job file behind: the `--timeout` budget kept running across the two RFC 7671 TTL waits (7200s each at the default TTL) and the `--reload` command, so the deadline had passed hours before prune opened its connection. The budget now bounds network work rather than process lifetime, and is re-armed after each wait and after the reload
