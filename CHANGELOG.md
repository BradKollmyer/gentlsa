...
- `--mx` on `generate`, `verify`, and `prune` looks up the zone's MX RRset and operates on each exchange host (lowest preference first), so DANE-for-mail no longer needs a `--hostname` per MX. The MX RRset is DNSSEC-validated as well (RFC 7672 §2.2): whoever forges an insecure MX answer picks the host and can then publish a properly signed TLSA for a key they hold, so `verify --mx` reports an unauthenticated MX RRset as WARNING and a bogus one as CRITICAL even when the TLSA records validate (`mx_dnssec` in `--json`). Out-of-zone exchanges are checked but cannot be published into the zone, a null MX (RFC 7505) is skipped, and an unreachable exchange is reported and skipped rather than aborting the run
- `--starttls smtp|imap|pop3|xmpp|none` selects the plaintext upgrade independently of the port on `generate`, `verify`, `list`, `prune`, and `rollover`. When omitted, 25/587 are SMTP, 143 IMAP, 110 POP3, and 5222/5269 XMPP; every other port stays implicit TLS. `--starttls none` forces implicit TLS on a STARTTLS port. XMPP sends the zone as the stream's `to=` attribute and as SNI, which RFC 6120 §4.7.2 requires to be the XMPP domain rather than the SRV target
- `--timeout <SECONDS>` (default 30) bounds hostname resolution, TCP connect, STARTTLS, the TLS handshake, and DNS, so a Nagios `verify` finishes before the service check timeout; TCP connect was previously unbounded. It is a true deadline rather than a per-operation one — every read and write is re-armed to what remains of the budget — and it bounds network work rather than process lifetime, so it is re-armed across `rollover`'s TTL waits and its `--reload` command
...
- Fix `verify` reporting a false `ERROR` against a zone that publishes only DANE-TA `2 1 1`, or any other non-`3 1 1` parameters: each DNS record is now evaluated with its own parameters against the presented chain (usage 1/3 against the leaf, 0/2 against any presented certificate, selectors 0/1, matching 0/1/2)
...
- Fix `generate` and `prune` overwriting the exit code across a port list, so a failure on one port could be masked by a later success
